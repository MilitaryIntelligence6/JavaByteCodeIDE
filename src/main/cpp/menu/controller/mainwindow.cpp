...
// 如果不改动 ui 文件依赖, 那么需要把 ui 文件和 controller 放一起以便编译生成在该目录下
...
MainWindow::MainWindow(QWidget *parent) :
    ...
    ui->mainToolBar->setStyleSheet(
            "QToolButton:hover {background-color:darkgray} QToolBar {background: rgb(82,82,82);border: none;}");
    //--------------------------------
...
void MainWindow::resizeEvent(QResizeEvent *event) {
    ...
    ui->editor->setGeometry(10, 0, width() - 20,
                            height() - ui->statusBar->height() - ui->mainToolBar->height() - 80 - 15);
...
void MainWindow::openFile() {
    if (!fileSaved) {
        if (QMessageBox::Save ==
            QMessageBox::question(this, tr("文件未保存"), tr("当前文件没有保存，是否保存？"), QMessageBox::Save, QMessageBox::Cancel))
            saveFile();
    }
...
void MainWindow::run() {
    ...
    if (!fileSaved) {
        if (QMessageBox::Save ==
            QMessageBox::question(this, tr("文件未保存"), tr("文件保存后才能运行，是否保存？"), QMessageBox::Save, QMessageBox::Cancel))
            saveFile();
    }
    if (fileSaved) {
        ...
        //qDebug()<<buildPath;
        process.start("bash", QStringList() << "-c" << QString(
                tr("g++ ") + filePath + tr(" -o ") + buildPath + tr(";") + buildPath));
...
void MainWindow::closeEvent(QCloseEvent *event) {
    if (!fileSaved) {
        if (QMessageBox::Save ==
            QMessageBox::question(this, tr("未保存就要退出？"), tr("当前文件没有保存，是否保存？不保存文件改动将会丢失"), QMessageBox::Save,
                                  QMessageBox::Cancel))
            saveFile();
        fileSaved = true;
    }
